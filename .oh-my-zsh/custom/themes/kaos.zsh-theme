...
git_super_status() {
    ...
    if [ -n "$__CURRENT_GIT_STATUS" ]; then
      STATUS="$ZSH_THEME_GIT_PROMPT_PREFIX$(git_repo_name)/$ZSH_THEME_GIT_PROMPT_BRANCH$GIT_BRANCH%{${reset_color}%}"
...
PROMPT='%F{208}%n%f%{$fg[white]%}@%F{039}%M%f%{$reset_color%}:%B%{$fg[green]%}%~%{$reset_color%} $(git_super_status) 
%(!.#.>) '
